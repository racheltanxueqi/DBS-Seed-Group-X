...
export default class AccountDetails extends Component {
  ...
  handleChange = (e) => {
    this.setState({
      ...
      accountView: this.state.accounts.find(
    ...
    return (
      <div>
        <div className="card border">
          <h1>Accounts</h1>
          <h2>
            Hi, <span className="blue">{this.state.login.firstName}</span>
          </h2>
        ...
        <div className="card border spread">
          <div>
            {this.state.accountView.accountName}
            <br></br>
            {this.state.accountView.accountNumber}
          </div>
          <div>
            {this.state.accountView.availableBal
              ? this.state.accountView.availableBal.toFixed(2)
              : 0.0}
          </div>
        </div>
